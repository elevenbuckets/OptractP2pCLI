...
const create = (password, datadir) => 
{
	cluster.setupMaster({exec: path.join(__dirname, './create.js')});

	if (cluster.isMaster) {
    		const __creates = (resolve, reject) => {
			const worker = cluster.fork({password, datadir}); // passing via env safer than IPC?
...
class KnifeIron {
	constructor(cfgObj) 
	{
		...
		this.validPass = () =>
	        {
	               ...
	               return this.newArchive(pw).then((rc) => {
		...
		this.newArchive = (masspw) => 
		{
			if (this.archfile === null) return Promise.reject(); //TODO: set a default location?
			if (fs.existsSync(this.configs.passVault)) return Promise.resolve(false);

			let myArchive = new bcup.Archive();
			let myGroup = myArchive.createGroup("ElevenBuckets");
			this.ds = new FileDatasource(this.configs.passVault);

			return this.ds.save(myArchive, createCredentials.fromPassword(masspw)).then(() => { return true; });
		}
		...
		this.newAccount = (password) => 
		{
		    	...
		    	return this.newArchive(pw)
	  	        .then( () => { return create(password, this.configs.datadir); })
		        .then( (result) => 
			{
		    		let ds = new FileDatasource(this.configs.passVault);
		...
		this.importFromJSON = (jsonpath, password) => 
		{
					...
					worker.on('message', (keyObj) => { 
						if (Object.keys(keyObj).length === 0) {
							...
							const __update = (resolve, reject) => {
								keth.exportToFile(keyObj, path.join(this.configs.datadir, 'keystore'), (path) =>
	        						{
	    						...
	    						return new Promise(__update).then(() => {
								ds.load(createCredentials.fromPassword(pw)).then( (myArchive) => 
		    						{
		    							let vaults = myArchive.findGroupsByTitle("ElevenBuckets")[0];
							    		vaults.createEntry(address)
					  	    		              .setProperty("username", '0x' + address)
	    							...
	    							.then( (myArchive) => 
	    							{
							    		return ds.save(myArchive, createCredentials.fromPassword(pw)).then( () => {
										console.log(`DEBUG: imported ${address} via ${jsonpath}`)
										resolve(true);
									});
		    						})
							})
		...
		this.managedAddress = (address) =>
	        {
		       ...
		       if (this.archfile === null) this.newArchive(pw);

		       stage = stage.then(() => {
				return this.allAccounts().then((list) => { return list.indexOf(address) !== -1 });
		       })

	               return stage.then((rc) => {
			      if (rc === false) return {[address]: false};
		...
		this.switchProvider = (newProvider) =>
		{
			...
			try {
				...
				console(`Connected to network with wrong ID, badRPC: ${newProvider}`);
				this.web3.setProvider(new Web3.providers.HttpProvider(this.rpcAddr));
				return false;
			}
		}
		...
		{
			cluster.setupMaster({ exec: path.join(__dirname, './slave.js') });
			if (cluster.isMaster) {
				let pw = masterpw.get(this).passwd;
				...
				return this.ds.load(createCredentials.fromPassword(pw)).then( (myArchive) => {
					...
					*/
						// Using Worker Threads to work with keythereum
								...
								worker.send({addr, datadir: this.configs.datadir, passes, data: msgSHA256Buffer, networkID: this.networkID}) 
		                	                ...
		                	                this.jobQ[Q][addr].map((o, id) => 
							{
								try {
									...
									nonce = nonce + id;
										...
										tx = new EthTx({ ...o.txObj, nonce, data}, {chain: this.networkID});
									} else {
										tx = new EthTx({ ...o.txObj, nonce}, {chain: this.networkID});
									}
									tx.sign(p.pkey);
		...
		this.hotGroups = tokenList =>
	        {
				...
				return this.connect().then((rc) => { 
					if (rc) this.hotGroups(tokenList); 
				})
			}
		...
		this.addrTokenBalance = tokenSymbol => walletAddr =>
	        {
	                ...
	                return this.call('Token')(tokenSymbol)('balanceOf')(walletAddr);
	        }
		...
		this.sendTk = (appName) => (ctrName) => (callName) => (...__args) => (amount = null) =>
                {
                        let tkObj = {};
			__args.map((i,j) => { tkObj = { ...tkObj, ['arg'+j.toString().padStart(2, '0')]: i } });
			...
			try {
        		...
        		try {
                		return Promise.resolve(this.enqueueTk(appName, ctrName, callName, appArgs)(fromWallet, amount, gasAmount, tkObj))
					      .then((jobObj) => { return this.processJobs([jobObj]).then(this.queueReceipts) })
		...
		this.sendTx = (tokenSymbol, fromWallet, toAddress, amount) =>
		{
			let jobObj = {};
			let gasAmount = 5;
			...
			try {
				return Promise.resolve(this.enqueueTx(tokenSymbol)(fromWallet, toAddress, amount, gasAmount))
					      .then((jobObj) => { return this.processJobs([jobObj]).then(this.queueReceipts) })
					      .catch((err) => { console.trace(err); return Promise.reject(err); }); // single job, thus single element in list
