...
[
        ...
        {name:     'oid', length: 32,   default: Buffer.from([]) },                   // participating game round ID, bytes32
...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
			...
			this.memberStatus(account).then((rc) => { return rc[0] === 'active'; }).then((rc) => {
				...
				this.getOproundInfo().then((rc) => {	
					...
					if (v1leaf === '0x') { // curate
						...
						//verify aid
						let _aid = ethUtils.bufToHex(ethUtils.sha256(Buffer.concat([titleBuf, Buffer.from(domain)])));
						if (aid !== _aid) {
		...
		this.newArticle = (url, tags = []) => (comment = "Optract by 11BE") =>
		{
