				...
				//   - sDB of previous opround, if any
	                        //   - this.db.acc2vc
                                ...
                                Object.keys(this.db.acc2vc).map((acc)=>{
                                        if (typeof(this.game.lastSrates[acc]) === 'undefined') {
                                                this.game.lastSrates[acc] = [0, this.db.acc2vc[acc]];
                                        } else {
			...
			return __endOpRound().then((rc)=>{
				...
				console.log('opround status id: ' + rc);
					...
					return Promise.all(p).then((rc)=>{
						...
						let finalistIPFS = '0x1';  // tmp solution before smart contract update
						let successRateDB = '0x1';  // tmp solution before smart contract update
