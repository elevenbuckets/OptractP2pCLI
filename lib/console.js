...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
				...
				this.getOproundInfo().then((rc) => {	
					...
					if (v1leaf === '0x') { // curate
						...
						if (typeof(this.aidWatch[aid]) === 'undefined') {
							this.aidWatch[aid] = { curated: [account], voted: [] }
					...
					if (v1leaf !== '0x') {
						...
						if (v2leaf === '0x') { // v1 vote
							return __add_abi('v1')(labels, values, v1.proves, v1.sides, v1.leaf, v1.block).then((abiObj) => 
							{
								...
								// verify double vote
								return this.locateTx(v1.block)(v1.leaf, 'aid').then((ans) => {
									if (typeof(this.aidWatch[aid]) === 'undefined') {
										this.aidWatch[aid] = { curated: [], voted: [account] }
		...
		this.locateTx = (blockNo) => (txhash, field) =>
		{
			if (field === 'aid' && typeof(this.db.block[blockNo]) !== 'undefined') {
				return Promise.resolve(this.db.block.tx2aid[txhash]);
			} else if (field === 'account' && typeof(this.db.block[blockNo]) !== 'undefined') {
				return Promise.resolve(this.db.block.tx2acc[txhash]);
			} else {
		...
		this.genOpRoundCache = () => 
		{
			// Recommend to make separate Block & OpRound caches
			// Understand their relationship before attempt to merge them.
			// TODO:
			// aid => [accounts] // curated
			// aid => [accounts] // voted
			// account => {v2_txhash, block_included} // claimed
			this.getOproundInfo().then((rc) => {
