...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		this.categoryKeywords = {  // use only lower case; include part of domain names to make sure all url from allFeeds are categorized
		    'tech': ['hacker', 'onezero'],
		    'finance': ['finance', 'bond', 'economics', 'bloomberg', 'reuters'],
		    'blockchain': ['ethereum', 'libra', 'bitcoin', 'btc', 'defi', 'dapp']
		}
		...
		this.newBotArticles = (_comment = "send by 11be bot") =>
		{
			...
			let workload = 0; let maxWorkload = 10;
			...
			Promise.all(p).then((rc) => {
				...
				feedInfoList.map((feedInfo, idx) => {
					...
					workload += 1;
					if (workload > maxWorkload) return;
					console.log(workload + tags + title + domain);
					setTimeout(this.newArticle, 15000*workload, url, tags, _comment);
				})
			})
