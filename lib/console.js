	...
	constructor(cfgObj) {
			...
			return Promise.all(p).then((rc) => {
				...
				rc.map((a) => {
					if (Object.keys(a).length != 0) {
						feedInfoList = [...feedInfoList, ...a.slice(0, 30)]
					}
				});
				let feedInfos = feedInfoList.map((feedInfo, idx) => {
					// TODO: unshorten url
				...
				return feedInfos.filter((ele)=>{return typeof(ele) !== 'undefined' && Object.keys(ele).length != 0});
			})
		}
		...
		this.newBotArticles = (_comment = "send by 11be bot") =>
		{
			...
			this.getRssArticles().then((rc)=>{  // rc = [{url, tags}, {url, tags}, ...]
				// for test, randomly choose some articles
				shuffleArray(rc);
				rc.slice(0, 12).map((urltags, i)=>{
