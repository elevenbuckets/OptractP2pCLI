				...
				console.log('opround status id: ' + rc);
					...
					return Promise.all(p).then((rc)=>{
						...
						return this.sendTk(this.appName)('BlockRegistry')('submitMerkleRoot')(
						    merkleRoot, ipfscid, snapshot[0].length, v1count, v2count, minSuccessRate, baseline, successRateDB, finalistIPFS, aidMerkleRoot, aidIpfscid)();
					})
                                } else if (rc === 1) {  // NDR
					let baseline = 0;
					let p = [__genblk(), __genAidBlk()];
