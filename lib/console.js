	...
	constructor(cfgObj) {
		...
		this.newBotArticles = (_comment = "send by 11be bot") =>
		{
			...
			this.getRssArticles().then((rc)=>{  // rc = [{url, tags}, {url, tags}, ...]
				...
				rc.slice(0, 10).map((urltags, i)=>{
					setTimeout(this.newArticle, 9000*i, urltags.url, urltags.tags, _comment);
				})
			})
		...
		this.makeMerkleTreeAndUploadRoot = () =>
			...
			const __genAidBlk = () =>
			{
				...
				let aidMerkleRoot = this.makeMerkleTree(aidList);
	 ...
	 .then(() => { return new Promise(askMasterPass) })
		...
		if (rc && typeof(opt.appCfgs.dapps[opt.appName].account) !== 'undefined') {
			return opt.linkAccount(opt.appName)(opt.appCfgs.dapps[opt.appName].account)
				  ...
				  .catch((err) => { title = 'Optract: Ops Console [ NA ]'; })
		} else {
