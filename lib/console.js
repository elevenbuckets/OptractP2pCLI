...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
				...
				Promise.all(p).then((rc) => {
					...
					if (this.myEpoch < newEpoch) {
						if (newOpRnd > this.game.opround) {
							let msr   = rc[2][3].toNumber();  // min success ratee
							let sDB   = rc[2][4];
							let fIPFS = rc[2][5];
							let oldOpRnd = this.game.opround;

							//reset this.game, pull sDB and fIPFS
		...
		{
			this.game = { drawed: false, opround: tkObj.started,
				      aid2vc: {}, aid2cc: {}, aidUrl: {},
				      curated: {}, voted: {}, votWatch: {},
				      clmWatch: {}
			};
			...
			let p = [ this.get(tikObj.lastSDB), this.get(tikObj.fIPFS) ];
			return Promise.all(p).then((rc)=>{
