	...
	constructor(cfgObj) {
		...
		this.allFeeds = [  // manually manage this for now
			...
			{url: 'https://medium.com/feed/one-zero', fields: {title: 'title', link:'link', contentSnippet:'contentSnippet'}}
                ...
                this.categoryKeywords = {  // use only lower case; include part of domain names to make sure all url from allFeeds are categorized
		    'tech': ['hacker', 'onezero'],
		    'finance': ['finance', 'bond', 'bloomberg', 'reuters'],
		    'blockchain': ['ethereum', 'libra']
		}

		this.newBotArticles = (_comment = "send by 11be bot") =>
		{
			let p = this.allFeeds.map((feed)=>{return this.parseFeed(feed)});
			return Promise.all(p).then((feedInfoList) => {
				return feedInfoList[0].map((feedInfo)=>{
					let url = feedInfo.link;
					let domain  = new URL(url).origin;
					let title = feedInfo.title;
					let aid = ethUtils.bufferToHex(ethUtils.sha256(Buffer.concat([Buffer.from(title), Buffer.from(domain)])));
					if (this.game.curated.hasOwnProperty(aid)) return;  // this aid exist in current opround
					const hasKeyword = (keywords, title) => {
						return keywords.some((keyword)=> {return title.includes(keyword)})
					}
					let tags = [];  // TODO: determine tags from contentSnippet or parsed content
                                    if (hasKeyword(this.categoryKeywords['tech'], title) || hasKeyword(this.categoryKeywords['tech'], url)) tags.push('tech');
                                    if (hasKeyword(this.categoryKeywords['finance'], title) || hasKeyword(this.categoryKeywords['finance'], url)) tags.push('finance');
                                    if (hasKeyword(this.categoryKeywords['blockchain'], title) || hasKeyword(this.categoryKeywords['blockchain'], url)) tags.push('blockchain');
					// Object.keys(this.categoryKeywords).map((k)=>{
                                            // if (hasKeyword(this.categoryKeywords[k], title) || hasKeyword(this.categoryKeywords[k], url)) {
                                                // // console.log('===>'+k + title + url);
                                                    // tags.push[k];
                                            // }
					// })
                                    // console.log(title); console.log(tags);
