...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
			...
			this.memberStatus(account).then((rc) => { return rc[0] === 'active'; })
				...
				if (v1leaf !== '0x') {
					...
					// insert v1proof , v1side
                                        ...
                                        let v1proves = [].concat.apply([], v1proof.slice(2).split('').map((v, k)=>{return k%64 ? [] : v.slice(k, k+64)}))
                                        let proofLength = v1proves.indexOf('0'.repeat(64));
                                        v1proves = v1proves.slice(0, proofLength);
                                        v1proves = v1proves.map((v)=>{return '0x'+v});

                                        let v1side = ethUtils.bufferToInt(data.v1side);
					let v1sides = v1side.split('').map((v)=>{return v==='1'? true : false});  // convert to bool array
                                        v1sides = v1sides.slice(0, proofLength);

                                        // verify
                                        this.call(this.appName)('BlockRegistry')('txExist')(v1proves, v1sides, v1leaf, v1block).then((rc1)=> {
                                                if (!rc1) return;
							...
							//TODO:
                                                        ...
                                                        this.call(this.appName)('BlockRegistry')('lotterySblockNo')(opround).then((rc2)=>{
                                                                ...
                                                                let v2proves = [].concat.apply([], v2proof.slice(2).split('').map((v, k)=>{return k%64 ? [] : v.slice(k, k+64)}))
                                                                let proof2Length = v2proves.indexOf('0'.repeat(64));
                                                                v2proves = v2proves.slice(0, proof2Length);
                                                                v2proves = v2proves.map((v)=>{return '0x'+v});

                                                                let v2side = ethUtils.bufferToInt(data.v2side);
                                                                let v2sides = v2side.split('').map((v)=>{return v==='1'? true : false});  // convert to bool array
                                                                v2sides = v2sides.slice(0, proof2Length);

                                                                // verify v2leaf in v2block
                                                                this.call(this.appName)('BlockRegistry')('txExist')(v2proves, v2sides, v2leaf, v2block).then((rc3) => {
                                                                        if (!rc3) return;
                                                                        ...
                                                                        // verify v1block[v1leaf] won opround "user" lottery && TODO: verify account did send v1leaf!
		...
		const __inner_msgTx = (content, txData) =>
		{
			...
			let oid; //TODO: need a constant function to query oproundLog ( opround id in bytes32)
		...
		this.newVote = (v1block, v1leaf, comments = '') =>
		{
			...
			let v2block = 0;
			let opround = this.game.opround;
                        let oid     = '0x'; // TODO: need to get oid

                        this.getProofSet(v1block, v1leaf).then((ps) => { // this will fail if v1leaf not in v1block
                                let v1proof = ps[0];
				let v1side  = ps[1];
		...
		this.newClaim = (v1block, v1leaf, v2block, v2leaf, comments = '') =>
		{
			let account = this.userWallet[this.appName];
                        let opround = this.game.opround;
                        let oid     = '0x'; // TODO: need to get oid
			let p = [ this.getProofSet(v1block, v1leaf), this.getProofSet(v2block, v2leaf) ];
...
*/
		...
		this.validateTx = (targetLeaf, sblockNo) =>
		{
			return this.getBlockData(sblockNo).then( (b) => {
				...
				return this.get(ipfsHash).then((d) => {
					let blockJSON = JSON.parse(d.toString())[0];
		...
		this.getProofSet = (targetLeaf, sblockNo) =>
		{
			return this.getBlockData(sblockNo).then( (b) => {
				let ipfsHash = Object.values(b.blockData)[0];
				// perhaps we could cache the block results??
		...
		this.parseBlock = (tikObj) =>
		{
			...
			this.getBlockData(newBlockNo - 1).then((blkdat) => { 
				...
				let bksnap = JSON.parse(rc[1].toString())[0].data;
				let mysnap = rc[2];
