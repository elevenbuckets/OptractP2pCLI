	...
	constructor(cfgObj) {
		...
		this.makeMerkleTreeAndUploadRoot = () =>
				...
				return Promise.all(p).then((rc) => {
					...
					if (rc[0] == 0) {  // genesis
						...
						if (now > v2EndTime + maxVoteTime) {  // no draw round
							...
							let v1count = Object.keys(this.game.aid2vc).length;
						...
						if (now > v1EndTime + maxVoteTime) {  // no draw round
							...
							let v2count = Object.keys(this.game.aid2cc).length;
			...
			const __genFinalistIPFS = (baseline) => 
			{
				...
				// aid to tx
				let finalistTx = [];
				this.game.aidwon.map((v)=>{finalistTx.push(this.game.aid2tx[v])});
				// tx to url
				let finalistURL = [];
				// TODO: get URLs
			...
			return __endOpRound.then((rc)=>{
				let v1count = Object.keys(this.game.votWatch).length;  // TODO: votWatch
				let v2count = Object.keys(this.game.clmWatch).length;  // clmWatch
