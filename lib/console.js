					...
					if (v1leaf !== '0x') {
						...
						// insert v1proof , v1side
	                                        let v1proof = ethUtils.bufferToHex(data.v1proof);  // assume v1proof is 0x + (32bytes)*16
                                                let v1proves = v1proof.slice(2).match(/.{1,32}/g).map((i)=>{return '0x'+i});
	                                        ...
	                                        let v1side = ethUtils.bufferToHex(data.v1side);
                                                let v1sides = v1side.slice(2).match(/.{1,2}/g).map((i)=>{return i === '00' ? false : true});
						// let v1sides = v1side.toString(2).split('').map((v)=>{return v==='1'? true : false});  // convert to bool array
	                                        ...
	                                        this.call(this.appName)('BlockRegistry')('txExist')(v1proves, v1sides, v1leaf, v1block).then((rc1)=> {
	                                                ...
	                                                values.splice(9, 0, v1proves);
	                                                values.splice(10, 0, v1sides);
