	...
	return connectRPC('ws://127.0.0.1:59437')
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			const __publishToStreamr = (apiKey, streamId, msg) => {
				let streamrClient = new StreamrClient({
					auth: {
						apiKey: apiKey  // with write access to the streamId
					}
				});
				return streamrClient.getOrCreateStream({id:streamId}).then((stream) => {
					return stream.publish(msg).then(()=>{
				...
				return opt.newVote(v1block, v1leaf, comment).then(() => {
					if (opt.appCfgs.streamr.enable === true) {  // then implement streamr
						return opt.locateTx(v1block)(v1leaf).then((txdata) => {
							...
							return __publishToStreamr(apiKey, streamId, msg);
						})
					}
