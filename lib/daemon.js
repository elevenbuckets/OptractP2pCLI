	...
	constructor(cfgObj) {
		...
		const __inner_msgTx = (comment, txData) =>
		{
			...
			return this.unlockAndSign(account)(Buffer.from(payload)).then((sig) => {
				...
				return __incomming({data: rlp.serialize()});
		...
		this.genOpTicketCache = () =>
		{
			if (!this.dbsync() || typeof(this.userWallet[this.appName]) !== 'undefined') return {};
			let output = {
				opround: this.game.opround, 
				oid: this.game.oid,
                                opStart: this.game.opStart,
				lottery: this.game.lottery,
				account: this.userWallet[this.appName],
				claimed: {}
			};

			return this.listAccLotteryWins(opround, addr).then((rc) => { 
				output['tickets'] = Object.keys(rc.voted).reduce((c, i) => { 
					rc.voted[i].map((j) => { c = { ...c, [j]: i } }) 
					return c;
				}, {})

				const __updatedb = (account) => (resolve, reject) => 
				{
					this.db.put(['tickets', account], output, (err, rc) => {
						if (err) reject(err)
						resolve(output);
					})
				}

				return new Promise(__updatedb(output.account));
			})
		}

		this.getOpTicketCache = () => 
		{
			if (!this.game.drawed || typeof(this.userWallet[this.appName]) !== 'undefined') return {};

			let account = this.userWallet[this.appName];

			const __fetchdb = (account) => (resolve, reject) => 
			{
				this.db.get(['tickets', account], (err, rc) => {
					if (err || (rc.constructor === Object && Object.keys(rc).length === 0)) reject(err)
					resolve(rc);
				})
			}

			return new Promise(__fetchdb(account));
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			const expose = 
			{
				...
				func: ['getBlockData', 'newAccount', 'newBotArticles', 'ping', 'queueReceipts', 'getOproundLottery', 'parseMsgRLPx', 'get', 'listCurationLotteryWins'],
			...
			r.register('getClaimArticles', (args) =>
			{
				...
				return opt.listCurationLotteryWins(opround).then((rc) => {
					return Object.keys(rc.aids).reduce((c,blk) => 
					{
						if (typeof(c[blk]) === 'undefined') c[blk] = [];
						if (rc.aids[blk].length > 0) c[blk] = [ ...c[blk], ...rc.aids[blk] ];
				...
				.then((c) => {
					...
					.then((rc) => {
						...
						for (let i = startBlk; i <= endBlk; i++) {
							p.push(new Promise(__bv1Content(i)))
						}

						return Promise.all(p).then((rc) => { return rc.reduce((o,i) => { o = {...o, ...i}; return o; }) });
					})
					.then((rc) => {
						return rc.reduce((o,i) => { o = {...o, ...i}; return o; });
					})
					.catch((err) => { console.trace(err); return {} })
				})
				.catch((err) => { console.trace(err); return {} })
