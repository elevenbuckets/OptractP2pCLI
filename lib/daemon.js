	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			r.register('streamrSwitch', () => 
			{
				...
				let outcfg = { ...opt.appCfgs }; 
				delete outcfg.dapps[opt.appName].account;

				//FIXME: not the best way to handle this ...
				fs.writeFileSync(path.join(opt.appCfgs.datadir, 'config.json'), Buffer.from(JSON.stringify(outcfg, 0, 2)));
