	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			const expose = 
			{
				...
				main: ['newArticle', 'newClaim', 'importFromJSON', 'listAccLotteryWins', 'sendTx'] // obj.args = [arg0, arg1 ...] (ordered args passed as object)
			}
			...
			const __surveyInfluence = (msg) =>
			{
				...
				if (!streamrSurvey instanceof StreamrClient) streamrSurvey = new StreamrClient({ auth: { apiKey }});
			...
			const __updateQuotes = (aid, ipfshash, account) =>
			{
				...
				return opt.get(ipfshash)
				   ...
				   .catch((err) => { quoteCache[aid].failed.push({[account]: ipfshash}) }); //FIXME: retry logic??
			}
					...
					try {
						if ( Math.abs(opt.myEpoch - Math.max(..._ac.queries[aid].blk)) < 5) {
							...
							return true;
							 ...
							 && Math.abs(opt.myEpoch - Math.max(..._ac.queries[aid].blk)) < 10 // for demo, should be 20
						) {
							if (typeof(_ac.queries[aid].page.content) !== 'undefined') delete _ac.queries[aid].page.content;
							return true;
						} else {
			...
			r.register('getClaimTickets', (args) => 
			{
				...
				if (typeof(this.game.lastSrates) === 'undefined') {
					console.log(`WARNING: lastSrates not found !!!!!!!!!!!!`);
					return {};
				}

				if (typeof(this.game.lastSrates[addr]) === 'undefined') {
					srate = 0;
				} else {
					srate = (this.game.lastSrates[addr][0] / this.game.lastSrates[addr][1]) * 100;
				}

				if (this.game.opround === 1 || this.game.lastMsr === 0) {
					true;
				} else if (this.game.opround > 1 && this.game.lastMsr > 0 && srate < this.game.lastMsr) {
					return {}; // MSR check
				}
			...
			r.register('dumpCaches', () => {
				return {articleCache, pickedCache, quoteCache}
			})
