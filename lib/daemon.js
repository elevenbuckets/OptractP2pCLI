			...
			r.register('newVote', (obj) =>
			{
				...
				return opt.newVote(v1block, v1leaf, comment).then(() => {
					if (opt.appCfgs.streamr.enable === true) {  // then implement streamr
						let streamId = 'PLr1IBJ3RYSkxHWN8WstNA';  // created via https://streamr.network/core/streams
						let streamrClient = new StreamrClient({
							auth: {
								apiKey: "a_eWnV7CSYKCqeJ8nE6gcAr1HWH1PVQ62b4niADM7RKA"  // with "write" access
							}
						});
						let p = [
						    streamrClient.getOrCreateStream({id:streamId}),
						    opt.locateTx(v1block)(v1leaf)
						]
						return Promise.all(p).then((rc) => {
							let stream = rc[0];
							let url = rc[1].url.toString();
							let aid = ethUtils.bufferToHex(rc[1].aid);
							const msg = {
								url: url,
								time: Math.floor(Date.now() / 1000),
								blkArticleId: v1block + aid  // of the voted article, '0x' is the delimiter
							}
							// console.log(`DEBUG: streamr data:`);
							// console.log(msg);
							return streamrClient.publish(stream.id, msg).then(()=>{
