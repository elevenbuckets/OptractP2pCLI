	...
	constructor(cfgObj) {
		...
		this.newVote = (v1block, v1leaf, comments = '') =>
		{
			...
			if (this.appCfgs.streamr.enable === true) {
				...
				p.push(streamrClient.getOrCreateStream({id:streamId}));
				p.push(this.locateTx(v1block)(v1leaf));
			}
				...
				if (rc.length > 2 && typeof(streamrClient) !== 'undefined') {
					let stream = rc[2];
					let url = rc[3].url.toString();
					let aid = ethUtils.bufferToHex(rc[3].aid);
					const msg = {
						userId: this.id,
						url: url,
						time: Math.floor(Date.now() / 1000),
						articleId: aid  // of the voted article
						// other fields: current block, v1blk, and?
					}
				    // console.log(`DEBUG: streamr data:`);
				    // console.log(msg);
					streamrClient.publish(stream.id, msg).then(()=>{
		...
		this.allFeeds = [  // manually manage this for now
		    ...
		    // https://www.tomsguide.com/feeds/all  (many reviews, but with some "deals in this month" articles)
                    ...
                    // http://www.reddit.com/domain/medium.com/.rss
