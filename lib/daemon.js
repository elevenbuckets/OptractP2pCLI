	...
	return connectRPC('ws://127.0.0.1:59437')
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			// streamr related (assume there's only one streamr key corresponding to one streamId)
			if (opt.appCfgs.streamr.enable === true) {
				opt.streamrOpts = {
					apiKey: "DyQSUVaSTyGp7R_lcGg1UgrQXNsu5AQtmdP52HoKEP-g",
					streamId: '31lQzneQQwicPuy_obROdg'
				}
				opt.streamrClient = new StreamrClient({
					auth: {
						apiKey: opt.streamrOpts.apiKey
					}
				});
				opt.streamrClient.on('connected', () => {
				    console.log('straemr: Yeah, we are connected now!')
				})
				opt.streamrClient.on('disconnected', () => {
				    console.log('streamr: ByeBye')
				})
			...
			r.register('blockBot', (args) =>
			{
				...
				if (typeof(bbTimer) === 'undefined') {
					...
					return `block bot is now off`
                        ...
                        // TODO: const __publishToStreamr = (apiKey, streamId, msg) => {
                        // TODO: const __publishVoteToStreamr(v1block, v1leaf);
			const __publishToStreamr = (msg) => {
				return opt.streamrClient.ensureConnected().then(()=>{
					return opt.streamrClient.getStream(opt.streamrOpts.streamId).then((stream) => {
			...
			r.register('newVote', (obj) =>
			{
				...
				return opt.newVote(v1block, v1leaf, comment).then(() => {
                                        // TODO: return __publishVoteToStreamr(v1block, v1leaf);
					if (opt.appCfgs.streamr.enable === true) {  // then implement streamr
						return opt.locateTx(v1block)(v1leaf).then((txdata) => {
							// let apiKey = "DyQSUVaSTyGp7R_lcGg1UgrQXNsu5AQtmdP52HoKEP-g"  // with "write" access
							// let streamId = '31lQzneQQwicPuy_obROdg';
						    ...
						    // console.log('DEBUG: __publishToStreamr params')
						    // console.log(msg);
							return __publishToStreamr(msg);
						})
					}
