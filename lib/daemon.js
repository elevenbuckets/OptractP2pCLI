	...
	constructor(cfgObj) {
		...
		const __incomming = (msg) => 
		{
			...
			Promise.all(fp).then((frc) => {
				...
				this.getOproundInfo().then((rc) => {	
					...
					let _txhash  = ethUtils.bufferToHex(ethUtils.keccak256(Buffer.from(this.abi.encodeParameters(txLabels, txValues))));
		...
		const __inner_msgTx = (comment, txData) =>
		{
			...
			return this.unlockAndSign(account)(Buffer.from(payload)).then((sig) => {
				...
				let txhash = ethUtils.bufferToHex(ethUtils.keccak256(Buffer.from(txHashPayload))); 
