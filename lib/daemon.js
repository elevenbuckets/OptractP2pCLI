...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		// Event related		
		this.myStamp = 0; //timestamp
		this.pending = { txdata: {}, payload: {}, txhash: {} };
		this.lostChunk = [];
		this.myNonce = 0;
		this.myEpoch = 0; // Optract block No.
		this.myTick  = 0; // Optract Epoch No.
		...
		this.setIncommingHandler((msg) => 
		{
			...
			this.memberStatus(account).then((rc) => { return rc[0] === 'active'; })
				...
				//console.dir(sigout);
					...
					this.showTxContent(txhash); // silenced. just to cache content on local ipfs.
	                        }
			    })
		})
		...
		this.getMerkleProof = (leaves, targetLeaf) => {
			...
			let bn = blkObj.myEpoch;
                        ...
                        const __genBlockBlob = (blkNo, blkbuf) => (resolve, reject) =>
                        {
                                ...
                                fs.writeFile(path.join(database, String(blkNo), 'blockBlob'), blkbuf, (err) => {
                                        if (err) return reject(err);
		...
		this.parseBlock = (tikObj) =>
		{
			...
			this.getBlockData(newBlockNo - 1).then((blkdat) => { 
				...
				this.pending = { txdata: {}, payload: {}, txhash: {} }; 
                                this.mergeSnapShot(bksnap, tokeep);
