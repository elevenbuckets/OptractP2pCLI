...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		this.allFeeds = [  // manually manage this for now
		    ...
		    // http://feeds2.feedburner.com/businessinsider  (contains articles from a broad range of categories)
		...
		this.getRssArticles = () =>
		{
			...
			const hasKeyword = (keywords, title) => {
				return keywords.some((keyword)=> {return title.includes(keyword)})
			}
				...
				const __new_block = () => 
				{
					if (opt.packSnap()[0].length > 0) {
						...
						return Promise.all(p).then((rc) => {
							...
							return opt.makeMerkleTreeAndUploadRoot().then((qid) => { return opt.queueReceipts(qid); })
						})
					}
				}
