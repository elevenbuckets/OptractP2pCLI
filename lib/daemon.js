	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			r.register('getClaimArticles', (args) =>
			{
				...
				return Promise.all(p).then((rc) => {
					return [ ...new Set([ ...Object.keys(rc[0].curated), ...Object.keys(rc[1].curated) ]) ].reduce((c,a) => 
					{
						if (typeof(c[a]) === 'undefined') c[a] = [];
						if (rc[0].curated[a].length > 0) c[a] = [ ...c[a], ...rc[0].curated[a] ];
						if (rc[1].curated[a].length > 0) c[a] = [ ...c[a], ...rc[1].curated[a] ];
