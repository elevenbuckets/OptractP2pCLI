...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		this.allFeeds = [  // manually manage this for now
			...
			{url: 'https://medium.com/feed/blockchain', category: ['blockchain'], fields: {contentSnippet:'content:encoded'}},
		    //finance
		...
		this.getRssArticles = () =>
		{
			const categoryKeywords = {
				...
				'tech': [
				    'technology', '/tech', 'hacker', 'onezero', 'slashdot', 'thechangelog', 'dzone%2fcloud',
				    'threatpost', 'renewable'
				],
				'emergingTech': [
				    'internetofthings', 'futurism', 'dzone%2fiot', 'robotic', 'artificial_intelligence',
				    'quantum comput', 'artificial intelligence'
				],
				'science': [
				    'science', 'space', 'nature.com', 'scientific', 'quantamagazine'
				],
				'blockchain': [
				    'blockchain', 'ethereum', 'bitcoin', 'btc', 'defi', 'dapp', 'crypto', 'decentral', 'libra',
				    'trustnodes', 'cointelegraph', 'blockcast'
				],
				'finance': [
				    'finance', 'economics', 'business', 'bond', 'nikkei', 'wealth', 'jpmorgan',
				    'fastcompany', 'bank', 'money', 'market', 'trade-war', 'trade war', 'economy'
                                ],
                                'investment': [
                                    'investment', 'kiplinger'
                                ]
			}
			...
			return Promise.all(p).then((rc) => {
				...
				rc.map((a) => {
					if (Object.keys(a).length != 0) {
                                                // TODO: deal with dates here, remove old ones (> 5 days?)
				...
				return feedInfoList.map((feedInfo, idx) => {
					...
					// assume feed title same as mercury-parser parsed title (there are exceptions)
					let aid = ethUtils.bufferToHex(ethUtils.sha256(Buffer.concat([Buffer.from(title), Buffer.from(domain)])));
