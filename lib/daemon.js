...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		this.listAccLotteryWins = (op, acc) =>  // return winning votes
		{
			...
			let output = { opround: op, account: acc, curated: {}, voted: {} };
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			r.register('getClaimArticles', (args) =>
			{
				...
				let p = [
				     opt.listAccLotteryWins(opround, '0xb440ea2780614b3c6a00e512f432785e7dfafa3e').catch((err) => { return {curated: {}} }), // curate bot 01
				     opt.listAccLotteryWins(opround, '0x4ad56641c569c91c64c28a904cda50ae5326da41').catch((err) => { return {curated: {}} }) // curate bot 02
				];
				...
				return Promise.all(p).then((rc) => {
					return [ ...new Set([ ...Object.keys(rc[0].aids), ...Object.keys(rc[1].aids) ]) ].reduce((c,a) => 
					{
						...
						if (rc[0].curated[a].length > 0) c[a] = [ ...c[a], ...rc[0].aids[a] ];
						if (rc[1].curated[a].length > 0) c[a] = [ ...c[a], ...rc[1].aids[a] ];
				...
				.then((c) => {
					let queue = [];
					Object.keys(c).map((blkNo) => {
						queue.push( new Promise(__range_queries(blkNo, blkNo, false)).then((rc) => { 
							return c[blkNo].map((aid) => { return rc[aid]; })
						})); 
					})
