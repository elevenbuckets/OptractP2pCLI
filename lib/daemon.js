...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		const __retry_pending = () =>
		{
			...
			if (typeof(this.pending.txhash[account]) === 'undefined'|| this.pending.txhash[account].length === 0) return;
			...
			dhashs.map((thash, i) => {
				...
				}, 3000*(i+1) + Math.random()*100, thash);
                        })
		}
				...
				__range(sbn, sblockNo - 1).map((b) => {
					return this.getBlockData(b).then((blkdat) => { 
						...
						return Promise.all(p).then((rc) => {
							...
							// gen block cache
							this.genBlockCache(b)(blkdat.ipfsHashes, bksnap, aid2tx, true);
						})
					})
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			r.register('getClaimTickets', (args) => 
			{
				...
				{
					let out = opt.db.get(['histxs'], (err, rc) => {
						if (err) return resolve({});
						return Object.values(rc).reduce((o,i) => {
