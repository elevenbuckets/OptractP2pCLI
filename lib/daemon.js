...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		this.getRssArticles = () =>
		{
			...
			let p = this.allFeeds.map((feed)=>{return this.parseFeed(feed)});

			return Promise.all(p).then((rc) => {
				let feedInfoList = [];
				// TODO: only recent 10 or 20 articles for each feed? Not all feed has 'date' field
				rc.map((a) => {  feedInfoList = [...feedInfoList, ...a] });
