	...
	constructor(cfgObj) {
		...
		const observer = (sec = 300000) =>
		{
				...
				Promise.all(p).then((rc) => {
					...
					if (this.myEpoch < newEpoch) {
						...
						if (this.game.opround === 0) { // when node just started
							this.getOproundLottery(newOpRnd).then((rc) => // no return here to emit epoch event 
							{
								opDraw = rc[1];
								this.game.drawed = (opDraw !== 0) ? true : false;
								this.game.lottery = opDraw;
								this.game.winNum = rc[2];
							})
						} else if (newOpRnd === this.game.opround && opDraw !== 0) {
							this.game.drawed = true;
							this.game.lottery = opDraw;
		...
		this.listAccLotteryWins = (op, acc) =>  // return winning votes
		{
			...
			return Promise.all(p).then((rc) => {
				...
				if (lotteryWinNumber === 0) return output;
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			r.register('locateTx', (obj) =>
			{
				...
				return opt.locateTx(blockNo)(txhash); // return the entire txdata
			})
