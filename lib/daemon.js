	     ...
	     if(!appCfg.daemon) {
			...
			const __init_cache = (startBlk, endBlk, biglist, picked) => (resolve, reject) =>
			{
				...
				if ( articleCache.constructor === Object 
				  ...
				  && endBlk >= articleCache.startBlk + 5
				) {
					// update
				...
				articleCache['aidlist']  = [ ...Object.keys(queries).sort() ];
				...
				Promise.all(mrq).then(() => { articleCache['queries'] = {...articleCache['queries'], ...newq}; resolve(true); })
			}
			...
			const __range_queries = (startBlk, endBlk, arCap = 15, parse = false) => 
			{
				...
				return Promise.all(queue).then((aclist) => {
					...
					new Promise(__init_cache(startBlk, endBlk, biglist, picked)).catch((err) => { console.trace(err); articleCache = {}; })
