	...
	return connectRPC('ws://127.0.0.1:59437')
	 ...
	 .then(() => {
	     if(!appCfg.daemon) {
			...
			if (opt.appCfgs.streamr.enable === true) {
				// for vote stream
				opt.streamr = {};
				opt.streamr.vote = __newStreamrClient({
					apiKey:"DyQSUVaSTyGp7R_lcGg1UgrQXNsu5AQtmdP52HoKEP-g",  // with "write" access
					streamId:'31lQzneQQwicPuy_obROdg'
				})
				// Looks like existing StreamrClient instance cannot change to another apiKey? If so, need to
				// create new instance with the corresponding apiKey for another stream, such as:
				// opt.streamr.survey = __newStreamrClient({ apiKey:'', streamId:'' })
			}


			const __publishToStreamr = (client, streamId, msg) => {
			...
			const __publishVoteToStreamr = (v1block, v1leaf) => {
				if (opt.appCfgs.streamr.enable === true) {  // then implement streamr
					return opt.locateTx(v1block)(v1leaf).then((txdata) => {
						...
						console.dir(msg);
						return __publishToStreamr(opt.streamr.vote.client, opt.streamr.vote.streamId, msg);
					})
				}
			}
